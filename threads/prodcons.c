...
sem_t empty; //cuantos tengo vacios al incio
sem_t fill; //cuantos tengo llenos al inicio
...
void * consumer(void *d){
	...
	while(1){
		...
		buffer[pos] = pos;
		pos = ((pos+1)%MAX_RECORDS);
		sleep(3);  //tiempo para consumir el valor
		printf("valor = %d \n", valor);
...
int main(){
	sem_init(&empty, 0, MAX_RECORDS); //Iniciar los semaforos
	sem_init(&fill, 0, 0);
	
	pthread_t producerThread, consumerThread;
	pthread_create(&producerThread, NULL, producer, NULL);
	pthread_create(&consumerThread, NULL, producer, NULL);
